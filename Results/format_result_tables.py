...
def create_table(datasets, experiment=None, model=None):
    allowed_experiments = ["flip-pertubation", "split"]
    ...
    if experiment == "flip-pertubation":
    ...
    for dataset in datasets:
        ...
        for sub_experiment in sub_experiments:
            ...
            for seed in seeds:
                eval_path = os.path.join(path_to_seeds, seed, "evaluation_test.txt")
            ...
            for table, metric in [(table_primary_metric, primary_metric), (table_secondary_metric, secondary_metric)]:
                table.loc[dataset, (*sub_experiment_key, "mean")] = np.mean(results[metric])
                table.loc[dataset, (*sub_experiment_key, "std")] = np.std(results[metric])
